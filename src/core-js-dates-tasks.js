...
function getNextFriday(date) {
  const newDate = new Date(date);
  const addDays = (5 - newDate.getDay()) % 7;
  newDate.setDate(newDate.getDate() + (addDays <= 0 ? 7 + addDays : addDays));
  return newDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const firstNewDate = new Date(dateStart);
  const secondNewDate = new Date(dateEnd);
  const seconds = Math.abs(firstNewDate.getTime() - secondNewDate.getTime());
  const days = Math.floor(seconds / (60 * 60 * 24 * 1000)) + 1;
  return days;
}
...
function isDateInPeriod(date, period) {
  ...
  const startPeriod = new Date(period.start).getTime();
  const endPeriod = new Date(period.end).getTime();
  if (startPeriod <= newDate && newDate <= endPeriod) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  const newDate = new Date(date);
  return `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, ${newDate.getUTCHours() > 12 ? newDate.getUTCHours() % 12 : newDate.getUTCHours()}:${String(newDate.getUTCMinutes()).padStart(2, '0')}:${String(newDate.getUTCSeconds()).padStart(2, '0')} ${newDate.getUTCHours() < 12 && newDate.getUTCHours() > 0 ? 'AM' : 'PM'}`;
}
...
function getCountWeekendsInMonth(month, year) {
  const overallDays = new Date(year, month, 0).getDate();
  let result = 0;
  for (let i = 1; i <= overallDays; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const yearStart = new Date(Date.UTC(getDate.getUTCFullYear(), 0, 1));
  const dayNum = getDate.getUTCDay() || 7;
  getDate.setUTCDate(getDate.getUTCDate() + 4 - dayNum);
  const weekNumber = Math.ceil(
    ((getDate - yearStart) / (60 * 60 * 24 * 1000) + 1) / 7
  );
  return weekNumber;
}
